...
create table if not exists regiao(
    ...
    loja_codigo int not null check (loja_codigo >= 0),
    foreign key (loja_codigo) references loja(codigo)
    );
...
create table if not exists gerente_regional(
    ...
    senha varchar(50) not null,
    regiao_codigo int not null check (regiao_codigo >= 0),
...
create table if not exists pedido(
    ...
    item_pedido_codigo int not null check(item_pedido_codigo >= 0),
    foreign key (item_pedido_codigo) references item_pedido(codigo),
...
create table if not exists contem(
    historico_codigo int not null check(historico_codigo >= 0),
    fluxo_codigo int not null,
    foreign key (historico_codigo) references historico(codigo),
    foreign key (fluxo_codigo) references fluxo(codigo)
);